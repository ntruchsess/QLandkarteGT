...
void CPicProcess::Hough(CPicProcess *ImgHough)
{
 ...
 //Pour tous les pixels , on trace le cosinus correspondant pondéré
...
void CPicProcess::Optimisation()
{
	...
	for(Pt.x=myWidth-1;Pt.x>=0;Pt.x--)
		for(Pt.y=myHeight-1;Pt.y>=0;Pt.y--)
			if(myGray[Pt.x][Pt.y]==(N-1)) //Si on est sur un pixel significatif
			{
				myGray[Pt.x][Pt.y]=0;
				...
				do
				{
					Pt=File.front(); //on le dépile et on regarde ses voisins
					File.pop();
					...
					for(i=-1;i<=1;i++)
						for(j=-1;j<=1;j++)
							if(Pt.x+i>=0 && Pt.y+j>=0 && Pt.x+i<myWidth && Pt.y+j<myHeight
							&& myGray[Pt.x+i][Pt.y+j]==(N-1))
							{
								Pt_Neighbor.x=Pt.x+i;
								Pt_Neighbor.y=Pt.y+j;
								myGray[Pt_Neighbor.x][Pt_Neighbor.y]=0;
								File.push(Pt_Neighbor); //on empile le pixel voisin
							}
				}
				while(!File.empty());
				if(Cpt!=0)
			...
			while(!Save.empty())
			{
				Pt=Save.front(); //on le dépile le pixel et on le marque
				Save.pop();
